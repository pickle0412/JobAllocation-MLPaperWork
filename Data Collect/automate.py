...
class Automate:
    def __init__(self, file_name, MAXSIZE, MINSIZE=1, type = 'm'):
        self.type = type
    ...
    def collect_data(self):
        for size in range(self.MINSIZE,self.MAXSIZE):
            self.size = size
            self.edit_source()
            self.compile()
    ...
    def save_df(self, df):
        ...
        try:
            df_old = pd.read_csv('DATA_test.csv')
            result = pd.concat([df_old, df])
            result.to_csv('DATA_test.csv', index=False)
        except:
    ...
    def mstring_modification(self, matrix):
        return np.array2string(matrix, threshold=1000000000,
                                    separator=',',
                                    ).replace(
                                            ']','}'
                                            ).replace(
    ...
    def random_generate_data(self):
        ...
        if(self.type == 'q' or self.type == 'l' or self.type == 'b'):
...
if __name__ == "__main__":
    a = Automate('linear/linear.cpp',10000,10000-6000, type = 'l')
    a.collect_data()
    a = Automate('bubble/bubble.cpp',10000, 1000, type = 'b')
    a.collect_data()
    a = Automate('quick_sort/quick_sort.cpp',1500, type = 'q')
    a.collect_data()
    a = Automate('matrix_mult/matrix_mult.cpp',600, type = 'm')
    a.collect_data()
    a = Automate('fib/fib.cpp',45, type = 'f')
    a.collect_data()
    # I am shutting the system off after it's done, good night
    os.system("poweroff")
